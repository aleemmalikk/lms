...
const CategoriesTab = ({ 
  ...
  const columns = [
    ...
    { 
      ...
      render: (item) => item.icon ? (
        <img src={item.icon} alt={item.name} className="h-10 w-10 object-cover rounded" />
      ) : (
    ...
    { 
      key: 'subcategories', 
      title: 'Subcategories',
      render: (item) => item.subcategories?.length || 0
    },
    { 
      key: 'required_fields', 
      title: 'Fields',
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex gap-2">
          {/* Form Builder Button for ALL Categories */}
          <button
            onClick={() => {
  ...
  const handleSaveCategoryForm = async (fields) => {
    try {
      ...
      console.log('Sending category update data:', updateData);

      const response = await fetch(
        `${BASE_URL}services/categories/${selectedCategory.id}/`,
        {
          method: 'PATCH',
          headers: {
      ...
      if (response.ok) {
        ...
        alert('Category form fields saved successfully!');
        ...
        alert('Failed to save category form fields: ' + JSON.stringify(errorData));
      }
    } catch (error) {
      console.error('Error saving category form fields:', error);
      alert('Error saving category form fields: ' + error.message);
    }
  };
  ...
  if (showFormBuilder) {
    return (
      <ServiceFormBuilder
        category={selectedCategory}  // ✅ category prop pass करें
        onSave={handleSaveCategoryForm}
        onCancel={() => setShowFormBuilder(false)}
        existingFields={selectedCategory.required_fields || []}
      />
    );
  }
